...
gr2<-sample_bipartite(80*nNodes/100, 20*nNodes/100, type="Gnm", m=nEdges, directed=TRUE, mode="in")
while(!is.dag(gr2))
  gr2<-sample_bipartite(80*nNodes/100, 20*nNodes/100, type="Gnm", m=nEdges, directed=TRUE, mode="in")

gr2<-edgeList(gr2)
names(gr2)=c(1:length(gr2))
func=function(n){runif(n,1,4)}
params=coef_gen(gr2, flip=FALSE)

dat6=generate_discrete_data(graph = gr2, n =nSamples, params = params)
while(1%in%auto_count_levels(dat6))
  dat6=generate_discrete_data(graph = gr2, n =nSamples, params = params)

dat6=list(dat6)
#gr1=list(gr1)
for (i in 2:nDatasets){
  dat6[[i]]=generate_discrete_data(graph = gr2, n = 50, params = params)
  while(1%in%auto_count_levels(dat6[[i]]))
    dat6[[i]]=generate_discrete_data(graph = gr2, n = 50, params = params)
}


trueDAG=gr2     #bipartite_edgeList2#bipartite_edgeList2#cytometryDiscrete$dag
...
for (pat in 1:nDatasets) {
  ...
  dgs=cd.run(data,adaptive=F,lambdas.length =2, error.tol= 1, convLb =0.2, upperbound = 60, alpha = 40, weight.scale = 0.001)
  #dgs=cd.run(data)
